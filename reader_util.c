 ...
 */
static int
search_for_pattern(int proto_id, uint8_t *payload){
    /* Search for pattern(s) and record */
    if(ndpi_match_string(pattern_match->automa[proto_id], payload)==1){
        return 0;
    }
    else
        return 1;
...
datalink_check:
    switch(datalink_type) {
        ...
        case DLT_EN10MB:
            ...
            /* Calculate ip pkt offset by eth pkt info */
...
iph_check:
    ...
    if(type == ETH_P_IP && header->caplen >= ip_offset) {
        ...
        proto = iph->protocol;
        /* Check capture length with header info */
