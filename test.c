...
void * processing_thread(void *_thread_id) {
    ...
    while(dpdk_run_capture) {
        ...
        const uint16_t nb_tx = rte_eth_tx_burst(dpdk_port_id^1, 0, bufs, num);
...
lcore_main(void)
{
    ...
    for(;;) {
        ...
        for (buf = nb_rx; buf < nb_rx; buf++){
            char *data = rte_pktmbuf_mtod(bufs[buf], char *);
            int len = rte_pktmbuf_pkt_len(bufs[buf]);
            struct pcap_pkthdr h;

            h.len = h.caplen = len;
            gettimeofday(&h.ts, NULL);

            ndpi_process_packet((u_char*)&ndpi_thread_info[0].pthread, &h, (const u_char *)data);
